...
const { user, state, isSubmitting, error, statusMessage, isAuthenticated } =
  storeToRefs(auth)
...
const memberSince = computed(() =>
  user.value
    ? new Intl.DateTimeFormat("en-CA", {
        month: "long",
        year: "numeric",
      }).format(new Date(user.value.date_joined))
    : "",
)
...
async function finishAuthentication(action: () => Promise<void>) {
  try {
    ...
    /* The store exposes a safe, user-facing error. */
  }
}
  ...
  <section class="page-shell account-page">
        ...
        <p>
          {{
            isAuthenticated
              ...
              : "Sign in for a smoother, more personal shopping experience."
          }}
        </p>
      </div>
      <div
        v-if="isAuthenticated"
        class="account-heading__mark"
        aria-hidden="true">
        OE
      </div>
    </header>
    ...
    <template v-else-if="!isAuthenticated">
      ...
      <div class="auth-grid">
        <form class="account-card" @submit.prevent="submitSignIn">
          ...
          <button
            ...
            {{
              isSubmitting && activeForm === "sign-in"
                ? "Signing in…"
                : "Sign in"
            }}
          </button>
        ...
        <form
          class="account-card account-card--accent"
          @submit.prevent="submitRegistration">
          <div class="account-card__heading">
            <p class="eyebrow">New here?</p>
            <h2>Create an account</h2>
            <p>
              Save your details now and be ready for order history and checkout.
            </p>
          </div>
          <div class="form-row">
            <div class="form-field">
          ...
          <div
            v-if="activeForm === 'register' && error"
            class="form-alert form-alert--error"
            role="alert">
            {{ error }}
          </div>
          <button
            ...
            :disabled="isSubmitting">
            {{
              isSubmitting && activeForm === "register"
                ? "Creating account…"
                : "Create account"
            }}
          </button>
          <p class="account-consent">
      ...
      <aside class="security-note">
        <strong>Security 1st</strong>
        <span>
          Passwords are hashed and stored securely on OrganicEmperor.com’s
          secure server.
        </span>
      </aside>
    </template>
    ...
    <template v-else>
      ...
      <div class="account-dashboard">
        <aside class="account-summary">
          ...
          >
          <RouterLink
            class="button account-summary__orders button--wide"
            :to="{ name: 'orders' }"
        ...
        <div class="account-settings">
          ...
          <form class="account-card" @submit.prevent="submitPassword">
            <div class="account-card__heading">
              ...
              <p>
                Your session stays active after a successful password change.
              </p>
            </div>
            <div class="form-field">
            ...
            <div
              v-if="activeForm === 'password' && error"
              class="form-alert form-alert--error"
              role="alert">
              {{ error }}
            </div>
            <button
              ...
              :disabled="isSubmitting">
              {{
                isSubmitting && activeForm === "password"
                  ? "Updating…"
                  : "Update password"
              }}
            </button>
